<?php
/*
Plugin Name:  Special Social Sharing icons
Version: 1.3
Description: Adds a set of social sharing after each post.
Author:   sherman-snow
Plugin URI: http://wordpress-plugins.us/special-social-sharing-icons-4/
*/
...
function social_sharing($content) {
    ...
    $simplesocial_dir = get_option('home').'/wp-content/plugins/special-social-sharing-icons/icons_'.get_option('ss_iconsize',32).'/';
    $simplesocialcontent = '';
...
function social_sharing_options(){
    ...
    foreach ($social_sharing_icons as $item) {
        if($item=='twitterwidget') {
            echo '<div style="margin-bottom:10px;font-weight:bold;font-size:9pt"><input style="margin-right:10px" id=cb_'.$item.' type="checkbox" size="20" name="ss_'.$item.'" '.(get_option('ss_'.$item,true)==true?'checked':'unchecked').'><label for="cb_'.$item.'"><img src="'.get_settings('home').'/wp-content/plugins/special-social-sharing-icons/img/twitterwidget.png"></label></div>';
        } else if($item=='facebookwidget') {
            echo '<div style="margin-bottom:10px;font-weight:bold;font-size:9pt"><input style="margin-right:10px" id=cb_'.$item.' type="checkbox" size="20" name="ss_'.$item.'" '.(get_option('ss_'.$item,true)==true?'checked':'unchecked').'><label for="cb_'.$item.'"><img src="'.get_option('home').'/wp-content/plugins/special-social-sharing-icons/img/facebookwidget.png"></label></div>';
        } else if($item=='googlepluswidget') {
            echo '<div style="margin-bottom:10px;font-weight:bold;font-size:9pt"><input style="margin-right:10px" id=cb_'.$item.' type="checkbox" size="20" name="ss_'.$item.'" '.(get_option('ss_'.$item,true)==true?'checked':'unchecked').'><label for="cb_'.$item.'"><img src="'.get_option('home').'/wp-content/plugins/special-social-sharing-icons/img/googlepluswidget.png"></label></div><div style="clear:both"></div>';
        } else{
            echo '<div style="float:left;margin-right:30px;margin-bottom:10px;"><input style="margin-top:-15px;margin-right:5px" id=cb_'.$item.' type="checkbox" size="20" name="ss_'.$item.'" '.(get_option('ss_'.$item,true)==true?'checked':'unchecked').'><label for="cb_'.$item.'"><img src="'.get_option('home').'/wp-content/plugins/special-social-sharing-icons/icons_32/'.$item.'.png"></label></div>';
        }
    }
...
function social_sharing_settinglink($links,$file) {
    if ($file=='special-social-sharing-icons/social-sharing.php') {
